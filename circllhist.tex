...
The counts $H(i)$ are represented 64 bit integers.
...
much better than 10\%, since the individual reconstruction errors even out across the dataset.
...
allocated bins never exceeded 1000, and the total size stays below 80kb.
...
floating point arithmetic (cf. Proposition \ref{prop:rec}).
It uses static branch annotations (GCC) to aid CPU branch predictions.
Moreover the quantile algorithm allows multiple quantiles to be computed with a single pass through the bucket list.
With these optimizations we can get raw insertion latencies down to $\sim 10ns$ for integer, and $\sim 80ns$ for double values.\footnote{
  These latencies were measured in a tight C loop with the provided \texttt{test/histogram\_perf.c} script, on a 2Ghz Intel Xeon CPU.
  The evaluation in section \ref{sec:eval} is Python based and uses different iteration counts and data.
}

Implemetations of the circllhist are available for a variety of languages including:
\begin{itemize}
