...
For the evaluation we choose three different datasets, each containing over 1 million samples, split
into more than 1000 batches.
...
\item API Latencies.  This dataset contains data collected at one of our internal APIs
  ...
  The data is spread between 0.29 and 11.000 (ms).
  There is a cutoff value around 11sec, that was caused by timeout logic getting triggered.
  This results in the higher quantile values being close together.
...
\item Simulated Latencies.
  ...
  The data is spread between on between $10^{-5}$ and $10^{10}$ with an extremely long tail.
...
Figures \ref{fig:size} and \ref{fig:tsize} show the sizes of the data structures after all samples
