...
\begin{abstract}
  ...
  It provides sub microsecond data insertion,
  full mergability,
...
Circonus has addressed this problem with the circllhist datastructure, that we describe in this document.
...
In the last years, the awareness of this problem has gradually grown and other vendors in the
Monitoring domain have started to address this problem as well.

A very similar approach to ours has been promoted and implemented by G. Tene from Aszul Systems
who developed a High Definition Range (HDR) Histogram data-structure \cite{hdr} to capture latency
data in Benchmarking applications.

Dunning and Erl developed the t-digest data structure in \cite{tdigest}, which is used in the
...
Here we focus on methods that are implemented in the above mentioned methods that are implemented in
monitoring products and have been widely used in practice.
